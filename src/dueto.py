...
class Dueto(Termo):
    ...
    def game_is_over(self):
        if self.winner:
            return True
        elif LetterPosition.empty not in self.board_1[-1][1] and\
            LetterPosition.empty not in self.board_2[-1][1]:
            return True
        return False
    ...
    def new_game(self):
        ...
        self.secret_words_unaccentuated = (
            remove_accents(self.secret_words[0]), #remove_accents(self.secret_words[1])
            "termo"
        )
